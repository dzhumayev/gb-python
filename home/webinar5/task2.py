...
def ask_about_of_candies_count():
    while True:
...
def take_candies(mode):
    if mode == "human":
        take_amount = ask_about_of_candies_count()
    ...
    if mode == "bot":
        ...
        if take_amount == 0:
            take_amount = randint(1, take_limit) if candies_amount[0] >= take_limit else take_limit
        candies_amount[0] = candies_amount[0] - take_amount
        return take_amount
...
while True:
    ...
    if move_transferred_to_player == 1:
        print("Ход делает игрок 1")
        print(f"Игроком взято конфет в количестве: {take_candies('human')}")
        print(f"Осталось конфет: {candies_amount[0]}\n")
        last_move_made_player = 1
        move_transferred_to_player = 2

    if move_transferred_to_player == 2:
        print("Ход делает игрок 2")
        mode = "bot" if player2_is_bot else "human"
        print(f"Игроком взято конфет в количестве: {take_candies(mode)}")
        print(f"Осталось конфет: {candies_amount[0]}\n")
        last_move_made_player = 2
        move_transferred_to_player = 1
